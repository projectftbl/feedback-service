...
var toArray = function(keys) {
  return (keys == null || _.isArray(keys)) ? keys : _.toArray(keys);
};

Repository.prototype.listByContentIds = function(contentIds, options) {
  var r = this.database;
  return this.find(function(feedback) {
    return feedback.hasFields('memberId').not().and(r.expr(toArray(contentIds)).contains(feedback('contentId')));
  }, options);
};
...
Repository.prototype.listByContentIdsForMember = function(contentIds, memberId, options) {
  var r = this.database;
  return this.find(function(feedback) {
    return feedback('memberId').eq(memberId).and(r.expr(toArray(contentIds)).contains(feedback('contentId')));
  }, options);
};
