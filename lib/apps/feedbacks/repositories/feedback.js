...
Repository.prototype.listByContentIds = function(contentIds, options) {
  ...
  return this.find(function(feedback) {
    return feedback.hasFields('memberId').not().and(r.expr(contentIds).contains(feedback('contentId')));
  }.bind(this), options);
};
...
Repository.prototype.listByContentIdsForMember = function(contentIds, memberId, options) {
  var r = this.database;
  return this.find(function(feedback) {
    return feedback('memberId').eq(memberId).and(r.expr(contentIds).contains(feedback('contentId')));
  }.bind(this), options);
};
